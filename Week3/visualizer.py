...
class TokenPredictor:
    ...
    def predict_tokens(self, prompt: str, max_tokens: int = 100) -> List[Dict]:
        ...
        response = self.client.chat.completions.create(
            ...
            temperature=2,  # Use temperature 1 for more diverse output
            logprobs=True,
            seed=42,
        ...
        for chunk in response:
            ...
            token = getattr(getattr(choice, "delta", None), "content", None)
            if not token:
                continue
            ...
            if logprobs is not None:
                ...
                if content:
                    logprob_items = getattr(content[0], "top_logprobs", None)
            ...
            logprob_dict = {item.token: item.logprob for item in logprob_items}

            # Get top predicted token and probability
            top_token = token
            top_prob = logprob_dict.get(token)
            if top_prob is None:
                top_prob = max(logprob_dict.values(), default=0.0)
